...
```
```
docker build -t customers-service:local .
```
```
docker network create ecommerce-net
```
```
docker run --rm --name customers-service \
  --network ecommerce-net \
  -e KAFKA_BOOTSTRAP_SERVERS=kafka:9092 \
  -p 9010:9010 \
  customers-service:local
```
Please be aware of the port being used in the docker command (9010) and the one configured in the
